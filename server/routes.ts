...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/email-bulk-send', requireAuth, async (req: Request, res: Response) => {
    try {
      const { recipients, subject, template, cc } = req.body;
      ...
      for (const recipient of recipients) {
        ...
        try {
          ...
          results.push({
            ...
            error: success ? undefined : 'Email service returned false'
          });
        } catch (error: any) {
          results.push({
            email: recipient.email,
      ...
      res.json({
        message: `Bulk email complete: ${successCount} sent, ${failCount} failed`,
        results
      });
    } catch (error: any) {
    ...
    try {
      ...
      const [updatedAssignment] = await db.update(schema.consumableAssignments)
        .set({
          assignedTo: updateData.assignedTo,
          serialNumber: updateData.serialNumber || null,
          knoxId:null, // Explicitly setting knoxId to null based on the original code
          notes: updateData.notes || null,
        })
        .where(eq(schema.consumableAssignments.id, assignmentId))
        .returning();
